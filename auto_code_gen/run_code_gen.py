...
def _find_latest_patch(output_dir):
    ...
    All patches are standalone (applicable on a clean base commit).
    Returns the most recently modified patch file.
    """
    _patch_pat = re.compile(
        r'^code_gen_(?:runtime|review)_V(\d+)\.patch$'
    )

    candidates = []
    for f in os.listdir(output_dir):
        if _patch_pat.match(f):
...
def _check_runtime_success(output_dir, current_patch_file=None):
    """Check if runtime iteration resulted in a successful benchmark run.

    If current_patch_file is provided, also verifies the success file was
    generated for this specific patch version (not an outdated one).
    """
    success_path = os.path.join(output_dir, RUNTIME_SUCCESS_FILE)
    if not os.path.isfile(success_path):
        return False
    if current_patch_file is None:
        return True
    with open(success_path) as f:
        content = f.read()
    if current_patch_file in content:
        return True
    print("  Note: existing success file is for a different patch version, re-running.")
    return False


def _make_model_runtime_dir(output_dir, model_name):
    """Create a model-specific runtime subdirectory name."""
    slug = re.sub(r'[^a-z0-9]+', '_', model_name.lower()).strip('_')
    return os.path.join(output_dir, "runtime_{}".format(slug))
...
async def run_pipeline(config, claude_config, use_case, resume=False):
    ...
    # Phase 6: Generate PR commands (optional)
    if config.generate_pr_commands:
        from auto_code_gen.code_gen_prompts import RUNTIME_SUCCESS_FILE
        any_success = any(
            os.path.isfile(os.path.join(output_dir, d, RUNTIME_SUCCESS_FILE))
            for d in os.listdir(output_dir)
            if os.path.isdir(os.path.join(output_dir, d)) and d.startswith("runtime_")
        ) or os.path.isfile(os.path.join(output_dir, RUNTIME_SUCCESS_FILE))
        if any_success:
            pr_results, pr_timings = await use_case.run_generate_pr_commands(
                context, config, claude_config,
            )
            phase_results.extend(pr_results)
            all_step_timings.extend(pr_timings)
        else:
            print("Skipping PR commands generation (no runtime succeeded)")
...
async def run_runtime_iterations(
    ...
    model_runtime_dir=None, execution_command=None, phase_label="Runtime",
):
    """Run the runtime iteration loop: apply patch, run benchmark, investigate.

    Results (logs, patches, success file) are written to model_runtime_dir
    if provided, otherwise to config.output_dir. Patches generated by the
    investigate step are written to config.output_dir (shared across models).

    Returns (phase_results, all_step_timings).
    """
    output_dir = config.output_dir
    results_dir = model_runtime_dir if model_runtime_dir else output_dir
    max_runtime_iterations = config.num_runtime_iterations
    runtime_logs_dir = os.path.join(results_dir, RUNTIME_LOGS_DIR)
    ...
    if resume and _check_runtime_success(results_dir, latest_patch):
        print("Resume: runtime iterations already succeeded for current patch (found {})".format(
            RUNTIME_SUCCESS_FILE
        ))
        return [{
            "name": phase_label,
    ...
    if not execution_command:
        print("ERROR: No execution command available.")
    ...
    print("\n" + "=" * 80)
    print("RUNTIME ITERATIONS: {}".format(phase_label))
    print("=" * 80)
    ...
    print("  Execution command: {}".format(execution_command))
    ...
    for i in range(iter_start - 1, max_runtime_iterations):
        ...
        if _check_runtime_success(results_dir):
            print("\nRuntime iteration {}: SUCCESS! Results written to {}".format(
                iteration, os.path.join(results_dir, RUNTIME_SUCCESS_FILE)
            ))
            runtime_succeeded = True
            break

        # Check if a new fixed patch was generated (patches are in output_dir, shared)
