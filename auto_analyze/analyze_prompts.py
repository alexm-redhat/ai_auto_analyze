...
class GpuOpsToTransformerBlocksPrompt:
    high_level_transformer_blocks_file: str
    gpu_ops_file: str
    output_file: str
    prompt_template: ClassVar[str] = """
[high_level_transformer_blocks_file] = {high_level_transformer_blocks_file}
[gpu_ops_file] = {gpu_ops_file}

[output_file] = {output_file}

- The file [high_level_transformer_blocks_file] describes different types of transformer blocks, and for each transformer block type it provides the sequence of high-level operations inside this block type.
- The file [gpu_ops_file] provides the sequence of low-level GPU operations that represent an execution of a sequence of transformer blocks, where each transformer block can be of a different type (from [high_level_transformer_blocks_file]).

Do the following plan and think hard (and ultra hard):
- Detect ranges of GPU operations inside [gpu_ops_file] that are full transformer blocks based on the high-level operations from [high_level_transformer_blocks_file]. Ensure no operation is missed, and that the start and end operations of each block are consistent.
- For each detected block, do the following:
    - Determine the transformer block type it represents
    - Correlate the sequence of low-level GPU operations of the block with the high-level operations from [high_level_transformer_blocks_file] based on the transformer block type. Ensure every low-level GPU operation is correlated with a high-level operation, while taking into account different GPU streams and their high-level implementation details.
...
"""
    ...
    def prompt(self):
        return self.prompt_template.format(
            high_level_transformer_blocks_file=self.high_level_transformer_blocks_file,
...
def gen_analyze_prompts(config: AnalyzeConfig):
    ...
    gpu_ops_to_blocks_prompt = GpuOpsToTransformerBlocksPrompt(
        high_level_transformer_blocks_file=transformer_block_high_level_prompt.output_file,
